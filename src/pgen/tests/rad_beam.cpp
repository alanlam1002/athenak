...
struct CrossingBeamData {
  ...
  // *inside* the domain (x0 > x1min) -- unlike the mechanism as originally
  // ported, which only ever (a) fills the whole downstream ray pattern once,
...
//! [Stage 12] Supports both the static-background <radiation> path and the
//! dynamical-spacetime <dyn_radiation> path (pmbp->pdynrad); see
//! FillCrossingBeams's source_radius handling above for why the continuous
//! interior re-injection is needed on top of the originally-ported mechanism.
