...
//  backward-Euler Compton exchange, linearizing the exchange rate at the
//  "star" (pre-coupling) state so the result reduces to a depressed quartic
//  in T_new -- mirrors the discrete-ordinate module's RadFluidCoupling
//  Compton solve (radiation_source.cpp:290-330), but built from M1's own
//  Compton rate law (radiation_m1_calc_opacities_photons.cpp:
//  sigma_compton = kappa_s*rho*4*T*inv_t_electron, driving J toward
//  arad*T^4). Assumes Primitive::IdealGas (T=P/rho, e_int=rho*T/gm1); safe
//  since compton=true+EOSCompOSE is already rejected upstream at parse time.
//
...
bool SolveComptonQuartic(const Real rho, const Real T_star, const Real Jstar,
                         const Real gm1, const Real kappa_s, const Real arad,
                         const Real inv_t_electron, const Real dtau,
                         const Real volform, Real &T_new, Real &J_new) {
  ...
  const Real one_plus_dtau_sigma = 1.0 + dtau * sigma_c_star;
  ...
  const Real coef4 = dtau * sigma_c_star * arad * volform / denom;
