...
def run_case(method, use_table=False, fd_step=FD_STEP, case=CASES[0]):
    label = (
        f"{case['name']}_{method}_"
        f"{'table' if use_table else 'analytic'}_"
        f"{fd_step:.0e}"
    )
...
def run_and_check(method, use_table=False, case=CASES[0]):
    ...
    check_against_reference(
        data,
        f"{case['name']} {method} {'table' if use_table else 'analytic'}",
        case)
    return data
...
def run_stress_suite():
    for case in CASES:
        for use_table in (False, True):
            ...
            compare_outputs(
                ad, fd,
                f"{case['name']} {'table' if use_table else 'analytic'} ad-vs-fd")
            subprocess.call("rm -rf tab *.dat *.hst", shell=True)
