...
//========================================================================================
//! \file gr_torus_m1.cpp
//! \brief M1 counterpart of ../fluids/gr_torus.cpp, forked (not modified in place, per
//! this codebase's own convention for radiation-backend variants -- see
//! gr_torus_flux.cpp/z4c_circumbinary.cpp, which fork the same torus math for their own
//! purposes) so the working DO/dyn_radiation comparison pgen is never put at risk.
//!
//! Everything hydro/MHD/torus-construction-related is byte-identical to gr_torus.cpp.
//! The only physics change is the radiation backend: gr_torus.cpp supports pmbp->prad
//! and pmbp->pdynrad (both angle-resolved discrete-ordinate arrays -- nangles/i0/nh_c/
//! tet_c); this file instead initializes pmbp->pradm1 (the M1 two-moment module, which
//! has no angular array at all -- just E/F^i). radiation_m1's own constructor hard-
//! requires an <adm> block to exist (radiation_m1.cpp: "radiation_m1 is only supported
//! with dynamical spacetimes"), which in turn means <mhd>+<adm> makes AddPhysics
//! construct pmbp->pdyngr (the dyn_grmhd solver) instead of the plain fixed-background
//! MHD solver gr_torus.cpp otherwise uses with <coord>-only GR -- gr_torus.cpp already
//! branches on pmbp->padm==nullptr for exactly this at the PrimToCons call at the end of
//! UserProblem, so this is a pre-existing, already-exercised code path, not new.
//!
//! The M1 radiation IC reuses the exact same physical quantity gr_torus.cpp already
//! computes for the DO backends -- the fluid-frame LTE energy density
//! urad = arad*T^4 -- and boosts it to the lab frame via the standard M1 optically-thick
//! (isotropic-in-the-fluid-frame) closure formula instead of a per-angle intensity sum:
//! given the Eulerian-observer Lorentz factor W and covariant spatial velocity u_i
//! (both already computed by the existing torus-velocity kernel):
//!   E_lab   = urad*(4*W^2 - 1)/3
//!   F_lab_i = urad*(4/3)*W*u_i
//! This is the isotropic radiation stress tensor R^ab = (4/3)*urad*u^a*u^b +
//! (1/3)*urad*g^ab projected onto the normal (Eulerian) observer -- the same physics
//! DO's per-angle boost encodes, just expressed as moments. See dyngr_bhstar.cpp for the
//! only other precedent of an M1 IC coupled to a dynamical-GR MHD pgen in this codebase
//! (a simpler, spherically-symmetric ad hoc flux-blend, not directly reusable here since
//! the torus has genuine angular momentum).
//!
//! Original file's own header, still accurate for everything not described above:
//! Problem generator to initialize rotational equilibrium tori in GR, using either
...
#include "mhd/mhd.hpp"
#include "radiation/radiation.hpp"  // needed for NoInflowTorus's pmbp->prad branch below,
                                    // which is never actually taken in this file's own
                                    // usage (pmbp->prad is always nullptr here) but still
                                    // needs the full type visible to compile
...
// NOTE: gr_torus.cpp's InitializeGRTorusRestartDynRadiation (a restart-time radiation
// reseed for runs that restart from a pre-radiation checkpoint) has no M1 counterpart
// here -- dropped, not adapted. M1 restarts carry pmbp->pradm1->u0 through the standard
// restart mechanism like any other evolved field; this pgen's fresh-start IC (below) is
// the only place M1's radiation state gets constructed from scratch.
...
void ProblemGenerator::UserProblem(ParameterInput *pin, const bool restart) {
  ...
  const bool is_radiation_enabled = (pmbp->pradm1 != nullptr);
  if (is_radiation_enabled) {
    // radiation_m1's own constructor already hard-requires <adm> to exist and exits
    // fatally if not (see this file's header comment) -- these checks catch the two
    // remaining ways this pgen specifically could be misconfigured, mirroring
    // dyngr_bhstar.cpp's precedent.
    if (pmbp->pmhd == nullptr || pmbp->pdyngr == nullptr) {
      std::cout << "### FATAL ERROR in " << __FILE__ << " at line " << __LINE__
                << std::endl << "gr_torus_m1 with radiation requires <mhd> and an "
                << "<adm>/<z4c> block (dyn_grmhd)." << std::endl;
      std::exit(EXIT_FAILURE);
    }
    if (pmbp->pradm1->params.opacity_type != radiationm1::Photons) {
      std::cout << "### FATAL ERROR in " << __FILE__ << " at line " << __LINE__
                << std::endl << "gr_torus_m1 requires <radiation_m1>/opacity_type = "
                << "photons (it builds the IC from photon_op_params.arad)."
                << std::endl;
      std::exit(EXIT_FAILURE);
    }
  }
  ...
  // Extract M1 radiation state if enabled -- no angular arrays needed at all (unlike
  // gr_torus.cpp's prad/pdynrad, M1 is a moment method: just E and F^i).
  DvceArray5D<Real> uradm1_;
  int nspecies_ = 0, m1_nvars_ = 0;
  radiationm1::RadiationM1Params m1_params_{};
  if (is_radiation_enabled) {
    uradm1_ = pmbp->pradm1->u0;
    nspecies_ = pmbp->pradm1->nspecies;
    m1_nvars_ = pmbp->pradm1->nvars;
    m1_params_ = pmbp->pradm1->params;
  }
  ...
  // Get radiation constant (if radiation enabled) -- pulled directly from the M1
  // opacity module's own resolved value (photon_op_params.arad, itself derived from
  // <photons>/arad or from <units> if that block exists), exactly like
  // dyngr_bhstar.cpp, so the IC and the evolved opacities can never silently diverge.
  if (is_radiation_enabled) {
    torus.arad = pmbp->pradm1->photon_op_params.arad;
    if (!(torus.arad > 0.0) || !std::isfinite(torus.arad)) {
      std::cout << "### FATAL ERROR in " << __FILE__ << " at line " << __LINE__
                << std::endl << "gr_torus_m1 requires a positive finite radiation "
                << "constant (<photons>/arad or <units>)." << std::endl;
      std::exit(EXIT_FAILURE);
    }
  }
  ...
  KOKKOS_LAMBDA(const int &idx, Real &max_ptot) {
    ...
    // Set M1 radiation moments (if radiation enabled). Standard optically-thick
    // (isotropic-in-the-fluid-frame) M1 closure: boost the fluid-frame LTE energy
    // density urad = arad*T^4 to the lab frame via the Eulerian-observer Lorentz
    // factor W and covariant spatial velocity u_i --
    //   E_lab   = urad*(4*W^2 - 1)/3
    //   F_lab_i = urad*(4/3)*W*u_i
    // (the projection of the isotropic radiation stress tensor
    // R^ab = (4/3)*urad*u^a*u^b + (1/3)*urad*g^ab onto the normal observer -- the exact
    // moment-space counterpart of gr_torus.cpp's per-angle boost above, same urad).
    if (is_radiation_enabled) {
      Real q = glower[1][1]*uu1*uu1 + 2.0*glower[1][2]*uu1*uu2 + 2.0*glower[1][3]*uu1*uu3
             + glower[2][2]*uu2*uu2 + 2.0*glower[2][3]*uu2*uu3
             + glower[3][3]*uu3*uu3;
      Real uu0 = sqrt(1.0 + q);
      Real u_1 = glower[1][1]*uu1 + glower[1][2]*uu2 + glower[1][3]*uu3;
      Real u_2 = glower[2][1]*uu1 + glower[2][2]*uu2 + glower[2][3]*uu3;
      Real u_3 = glower[3][1]*uu1 + glower[3][2]*uu2 + glower[3][3]*uu3;

      Real E_rad = urad*(4.0*SQR(uu0) - 1.0)/3.0;
      AthenaPointTensor<Real, TensorSymm::NONE, 4, 1> F_d{};
      // F_d(0) = beta^i F_i (pack_F_d's own documented convention, radiation_m1_tensors.hpp)
      // -- NOT 0.0 unconditionally. dyngr_bhstar.cpp's identical-looking pack_F_d(0.0, 0.0,
      // 0.0, ...) call is only correct there because that pgen's metric has beta^i=0
      // identically everywhere (isotropic Schwarzschild); this torus has a genuinely
      // nonzero shift (spinning Kerr-Schild, a!=0), so hardcoding 0.0 here silently
      // corrupted apply_floor's causality check (F_a F^a < (1-eps) E^2 collapses to
      // gamma^ij F_i F_j only when F_d(0) is set correctly) specifically in the
      // shift-nonzero region near the BH, letting an inconsistent (E,F) state through
      // the floor at t=0 that the closure/source-update solvers then blew up over the
      // next several cycles -- root-caused via direct comparison against every other
      // radiation_m1 call site, all of which thread the real shift through.
      // NOTE: deliberately NOT read from pmbp->padm->adm.beta_u here -- SetADMVariables()
      // (which fills it) isn't called until after this whole kernel finishes (see below),
      // so at this point in execution it's still all zero and reading it would silently
      // reproduce the exact same bug. Derive beta^i locally from the already-correct,
      // already-computed gupper instead (standard ADM relation g^0i/g^00 = -beta^i,
      // i.e. beta^i = -g^0i/g^00), matching the same gupper[0][*]/gupper[0][0] pattern
      // this file already uses just above to build uu1/uu2/uu3 from u1/u2/u3/u0.
      Real beta1 = -gupper[0][1]/gupper[0][0];
      Real beta2 = -gupper[0][2]/gupper[0][0];
      Real beta3 = -gupper[0][3]/gupper[0][0];
      pack_F_d(beta1, beta2, beta3,
               urad*(4.0/3.0)*uu0*u_1,
               urad*(4.0/3.0)*uu0*u_2,
               urad*(4.0/3.0)*uu0*u_3, F_d);
...
// (gr_torus.cpp's InitializeGRTorusRestartDynRadiation lived here -- dropped, see the
// file header comment and the note next to its prototype declaration above.)
