...
class Root {
 public:
  ...
  template<class Functor, class ... Types>
  KOKKOS_INLINE_FUNCTION
  bool FalsePosition_test(Functor&& f, Real &lb, Real &ub, Real& x, Real tol,
                     Types ... args) const {
    int side = 0;
    Real ftest;
    unsigned int count = 0;
    ...
    if (flb*fub > 0) {
      Kokkos::printf("RootFinding NotBracket xlu= %.17g %.17g, flu= %.17g %.17g\n",
        lb, ub, flb, fub);
      return false;
    }
    do {
      ...
      // Calculate f at the prospective root.
      ftest = f(x,args...);
      Kokkos::printf("RootFinding it = %d, x= %.17g, f= %.17g, xlu= %.17g %.17g, flu= %.17g %.17g\n",
        count, x, ftest, lb, ub, flb, fub);
      if (fabs((x-xold)/x) <= tol) {
        return true;
      }
      // Check the sign of f. If f is on the same side as the lower bound, then we adjust
