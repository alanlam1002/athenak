...
//         orbital-plane symmetry). Works under EITHER metric-evolution backend,
//         selected at runtime by which block is present in the .athinput file:
//           - <z4c>: writes alpha/beta_u into the Z4c host mirror and g_dd/vK_dd
//             into the ADM host mirror, then converts the ADM initial data to
//             Z4c's own evolved variables (ADMToZ4c) before returning.
//           - <adm> + <cfc>: writes an ADM-only initial guess (incl. an explicit
//             psi4 field, unused by z4c); CFC::InitializeMetric() (invoked
//             automatically by Driver::Initialize() right after this pgen
//             returns) then Picard-iterates that guess to a self-consistent
//             conformally-flat solution. No ADM->Z4c conversion in this case.
//         <z4c> and <cfc> are mutually exclusive (enforced with a FATAL ERROR in
//         src/mesh/meshblock_pack.cpp, independent of this pgen).
//
...
//  performing a single pass over all grid points. This import machinery, and the
//  fill loop itself, is entirely backend-agnostic: it only ever writes through
//  the shared `adm::ADM::ADMhost_vars host_adm` struct, whose individual field
//  storage is wired to the correct underlying array by the host-mirror setup
//  below (mirroring adm::ADM's own constructor logic in src/coordinates/adm.cpp).
//
...
void SetupBNS(ParameterInput *pin, Mesh* pmy_mesh_) {
  ...
  // =========================================================================
  // Host-mirror setup. adm::ADM's own constructor (src/coordinates/adm.cpp)
  // aliases alpha/beta_u into the Z4c class's u0 array when <z4c> is active,
  // or into u_adm itself otherwise; g_dd/psi4/vK_dd always live in u_adm
  // regardless. Mirror that exact logic here so the fill loop below can write
  // through the single, shared host_adm struct unconditionally -- it never
  ...
  // psi4 is unused by z4c but always a valid slice into u_adm regardless of
  // backend (I_ADM_PSI4 sits before I_ADM_ALPHA/BETA* in the enum, so it's
  // unaffected by z4c mode reserving 4 fewer channels in u_adm) -- write it
  // unconditionally; CFC requires it, z4c simply ignores it.
  ...
  Kokkos::parallel_for("kadath_fill",
    ...
    // Lapse and shift: an initial guess only -- CFC::InitializeMetric() (run
    // automatically from Driver::Initialize() right after this pgen returns)
    // Picard-iterates this to a self-consistent solution.
    ...
    // Spatial metric: g_ij = psi^4 * delta_ij (conformally flat, exactly as
    // CFC's own solver always enforces). psi4 is also stored as its own field
    // (I_ADM_PSI4), distinct from g_dd -- required by CFC, unused by z4c.
    Real g3d[NSPMETRIC];
    ...
    host_adm.psi4(m, k, j, i) = static_cast<Real>(psi4);

    // Extrinsic curvature: K_ij = psi^4 * A_ij. Also only an initial guess --
    // CFC::InitializeMetric()'s own solve (AssembleLapseShiftK) always
    // overwrites this, exactly as every other CFC pgen in this tree assumes.
  ...
  // Under CFC (is_z4c == false), no further conversion is needed here:
  // CFC::InitializeMetric() (called automatically from Driver::Initialize()
  // right after UserProblem() returns) takes it from here, Picard-iterating
  // the ADM initial guess just written above (holding these primitives
  // fixed) to a self-consistent solution.
}
...
//!        using either the z4c or the CFC metric solver (see file header).
void ProblemGenerator::UserProblem(ParameterInput *pin, const bool restart) {
  MeshBlockPack *pmbp = pmy_mesh_->pmb_pack;
  if (!pmbp->pcoord->is_dynamical_relativistic) {
    std::cout << "### FATAL ERROR in " << __FILE__ << " at line " << __LINE__
              << std::endl << "Celephais BNS problem requires an <adm> or "
              << "<z4c> block" << std::endl;
    exit(EXIT_FAILURE);
  }
  user_hist_func = &KadathBNSHistory;
  if (pmbp->pz4c != nullptr) {
    // AMR refinement condition delegates to Z4c AMR -- only meaningful (and
    // only safe to register) when z4c is actually active; under CFC,
    // user_ref_func stays unset, as with every other CFC pgen in this tree.
...
//! \brief AMR refinement condition (delegates to Z4c AMR). Only ever
//!        registered as user_ref_func when z4c is active (see UserProblem).
void KadathBNSRefinementCondition(MeshBlockPack *pmbp) {
