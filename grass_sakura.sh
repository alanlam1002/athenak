#!/bin/bash
#
# Build script for AthenaK with the dual z4c/CFC GRASS initial-data reader on Sakura,
# via the dyn_grmhd/dyngr_grass problem generator. The SAME binary serves both <z4c>
# and <cfc>/<adm> runs -- the mode is selected at runtime by which block is present in
# the .athinput file (see src/pgen/dyn_grmhd/dyngr_grass.cpp's file header). No
# external library dependency (plain C++ restart-binary reader), so no CMakeLists.txt
# changes were needed -- same toolchain/module set as cfc_celephais_ns_sakura.sh.
...
BUILD_DIR=${ATHENAK_ROOT}/build_grass
...
cmake \
  ...
  -DPROBLEM=dyn_grmhd/dyngr_grass \
