...
//         (no symmetry, supports a tilted spin axis). Works under EITHER
//         metric-evolution backend, selected at runtime by which block is
//         present in the .athinput file (see celephais_bns.cpp's file header
//         for the full explanation -- identical dispatch mechanism here):
//           - <z4c>: converts the ADM initial data to Z4c's own evolved
//             variables (ADMToZ4c) before returning.
//           - <adm> + <cfc>: leaves an ADM-only initial guess for
//             CFC::InitializeMetric() (invoked automatically right after this
//             pgen returns) to Picard-iterate to a self-consistent solution.
//         <z4c> and <cfc> are mutually exclusive (enforced in
...
//  the AthenaK fill loop. This import machinery, and the fill loop itself, is
//  entirely backend-agnostic -- see celephais_bns.cpp's file header for how.
//
...
void SetupNS(ParameterInput *pin, Mesh* pmy_mesh_) {
  ...
  // =========================================================================
  // Host-mirror setup. See celephais_bns.cpp's identical block for the full
  // explanation: adm::ADM's own constructor aliases alpha/beta_u into either
  // the Z4c class's u0 array (<z4c> active) or u_adm itself (CFC/bare-ADM);
  // g_dd/psi4/vK_dd always live in u_adm regardless. Mirror that logic here.
  // =========================================================================
...
//! \brief Problem generator for a single NS with Celephais (NS / NS_nosym GR
//!        apps), using either the z4c or the CFC metric solver (see file header).
void ProblemGenerator::UserProblem(ParameterInput *pin, const bool restart) {
  MeshBlockPack *pmbp = pmy_mesh_->pmb_pack;
  if (!pmbp->pcoord->is_dynamical_relativistic) {
    std::cout << "### FATAL ERROR in " << __FILE__ << " at line " << __LINE__
              << std::endl << "Celephais NS problem requires an <adm> or "
              << "<z4c> block" << std::endl;
    exit(EXIT_FAILURE);
  }
