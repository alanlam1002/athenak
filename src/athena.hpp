...
//
// 2026-09: lowered 14 -> 12. The MPICH on Aurora's next-eval image reports
// MPI_TAG_UB = 262143, i.e. only 18 bits, not the 20 the note above assumes. With
// NUM_BITS_LID=14 and bufid up to 56 neighbours the tag reaches 933,887, so every
// bufid >= 16 produced "Fatal error in internal_Irecv: Invalid tag" and killed the job
// one cycle after restart (192 ranks, measured 2026-09-08). At 12 bits the maximum tag
// is 56<<12 | 4095 = 233,471, which fits 18 bits, and the cap on MeshBlocks per rank is
// still 4096 -- against ~8 in production and a <mesh>/max_nmb_per_rank of 100. Tags are
// message labels only, so this changes no computed value: results stay bit-identical,
// and 18-bit tags are valid on every library that accepted 20.
