...
Mesh::Mesh(ParameterInput *pin) :
  ...
  coord_general(CoordinateGeneral::cartesian),
  one_d(false),
  two_d(false),
  three_d(false),
  multi_d(false),
  ...
  // Set grid-geometry coordinate system (default: cartesian). This is distinct from the
  // relativistic metric properties read from the <coord> input block (see CoordinateGeneral
  // enum docs in mesh.hpp).
  {
    std::string coord_str = pin->GetOrAddString("mesh", "coord", "cartesian");
    if (coord_str == "cartesian") {
      coord_general = CoordinateGeneral::cartesian;
    } else if (coord_str == "cylindrical") {
      coord_general = CoordinateGeneral::cylindrical;
  ...
  // error check mesh extents/topology against grid-geometry coordinate system
  // (see Mesh::ValidateCoordGeneral() below for details of each check)
...
void Mesh::ValidateCoordGeneral() {
  ...
  if (coord_general == CoordinateGeneral::cylindrical ||
    ...
    if (mesh_size.x1min == 0.0 && mesh_bcs[BoundaryFace::inner_x1] != BoundaryFlag::reflect) {
      std::cout << "### FATAL ERROR in " << __FILE__ << " at line " << __LINE__ << std::endl
