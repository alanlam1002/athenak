...
void PerformC2PTests(Mesh *pmesh, ParameterInput *pin) {
  ...
  KOKKOS_LAMBDA(const int /*dummy*/) {
    ...
    Kokkos::printf("  closed-form prediction 1-f_min ~ P_N/(Bag_B + P_N); "
                   "compare col 2\n");
    ...
    Kokkos::printf("\n--- Test F3: is the genuine mixed phase safe? "
                   "(P at f = f-min obs.)\n");
    ...
    {
      Kokkos::printf("\n--- Test G: C2P across the transition plateau "
                     "(embedded cells)\n");
      Kokkos::printf("  c2p_iter = %d, c2p_tol = %.3g\n",
                     ps_.GetRootSolver().iterations, ps_.tol);
      const Real gstates[6][18] = {
        {0.00043475771251898167, 2.6009584932404285e-05, 5.3512949404931102e-05,
            -2.4834079778579053e-06, 9.468640619370202e-06, 0.00043450583922652114,
            0.00043445354482703603, 1.9261263464817445e-05, -2.7240357366356378e-07, 0,
            0, 0, 1.7146247032735964, -0.0057249745572215778, -0.016154103865512581,
            1.7510860764961051, -0.036268727254053661, 1.709912250604622},
           // 1-f = 5.793e-04
        {0.00043472572878742316, 5.3123510303486221e-05, -2.6009804483774832e-05,
            1.0141593647907223e-06, 1.1865375302391179e-05, 0.00043365532129966864,
            0.00043338858319889271, 2.3315747930320253e-05, -1.1783856912121293e-06, 0,
            0, 0, 1.683273638150907, -0.017212724915685575, -0.039816724924499877,
            1.8060567882448626, 0.021359859969107432, 1.7404813866550435},
           // 1-f = 2.462e-03
        {0.00043531721273746101, 1.51812243186332e-05, -6.659609472174653e-05,
            2.7053198107580913e-06, 1.6393258709163403e-05, 0.00043287265482917943,
            0.00043247177435936577, 2.1915211063800995e-05, -2.8277634358198168e-06, 0,
            0, -0, 1.7605606289719247, 0.095500348557899323, 0.00087265682488049114,
            1.7925172472519832, -0.0045404556711991654, 1.7725654442390744},
           // 1-f = 5.616e-03
        {0.00043484468783775088, -3.6553941596465079e-06, 5.9346720203812824e-05,
            -4.6121771440021529e-06, 1.3044830384508817e-05, 0.00043113405648466092,
            0.00043041395777376144, 2.4802041417166189e-05, -4.0039453079399974e-06, 0,
            0, 0, 1.7646795470035141, -0.010221492054417837, -0.001192290989202484,
            1.6311158266641332, -0.036175279535092406, 1.7377426180506876},
           // 1-f = 8.533e-03
        {0.00043447862474202284, 4.9041700008720206e-05, 2.9192451289629175e-05,
            2.3616200245098488e-06, 1.3896042460211634e-05, 0.00042908452160823034,
            0.00042806470170674547, 2.6725299307927038e-05, -6.1543149668035717e-06, 0,
            0, 0, 1.808854537083262, 0.003675721592967801, -0.025132102248305529,
            1.736789748074522, -0.011505899854487123, 1.7296424658906417},
           // 1-f = 1.242e-02
        {0.00043503379897151111, 6.1627771771761853e-05, 6.1791616074533441e-06,
            -7.651782579729833e-06, 1.5044132392034945e-05, 0.00042679896234994798,
            0.00042520045638980218, 2.8669065563611428e-05, -7.234241372423834e-06, 0,
            0, 0, 1.6349851490100573, 0.0097208265555862924, -0.036485326300134791,
            1.7747140643096959, 0.0017974665640484312, 1.7452510237704277},
           // 1-f = 1.893e-02
      };
    ...
    // A is printed alongside so the two can be compared directly.
    {
      ...
      Kokkos::printf("  Test K: %d failure(s) -- y_lQ must be invariant under the "
                     "floor (expect 0)\n", kbad);
      Kokkos::printf("  A_pred is the OLD bias, shown for scale; it must no "
                     "longer occur.\n");
      Kokkos::printf("  (physical y_lQ range is [-0.679, 0]; species bound is "
                     "[-1, 2])\n");
    }
    ...
    // WENOZ overshoots at extrema. Then nQ < 0 flows into ColdPressureQuarks.
    {
      ...
      for (int e = 0; e < 6; ++e) {
        ...
        const bool fin = isfinite(P) && isfinite(en) && isfinite(nY[1]) &&
                         isfinite(nY[3]);
        if (!fin) { lbad++; }
